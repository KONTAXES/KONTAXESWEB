...
/* ─── Slide Theme Context ───────────────────────────────────────────────────
   PresentationViewer wraps slides in <SlideThemeContext.Provider value={isDark}>.
   Content slides (SLIDE_BG) call useT() to get themed tokens.
   Section/cover slides (GRAD_BG) are always dark — they don't use useT().
─────────────────────────────────────────────────────────────────────────── */

export const SlideThemeContext = React.createContext<boolean>(true); // true = dark

interface SlideTokens {
  isDark: boolean;
  bg: string;
  t1: string; t2: string; t3: string;
  a65: string; a60: string; a55: string; a50: string;
  a45: string; a40: string; a38: string; a22: string;
  s1: string; s08: string; s07: string; s06: string;
  s05: string; s03: string; s025: string; divider: string;
}

const DARK_T: SlideTokens = {
  isDark: true,
  bg: '#07030f',
  t1: '#f1f5f9', t2: '#e2e8f0', t3: '#cbd5e1',
  a65: 'rgba(255,255,255,0.65)', a60: 'rgba(255,255,255,0.60)',
  a55: 'rgba(255,255,255,0.55)', a50: 'rgba(255,255,255,0.50)',
  a45: 'rgba(255,255,255,0.45)', a40: 'rgba(255,255,255,0.40)',
  a38: 'rgba(255,255,255,0.38)', a22: 'rgba(255,255,255,0.22)',
  s1:  'rgba(255,255,255,0.10)', s08: 'rgba(255,255,255,0.08)',
  s07: 'rgba(255,255,255,0.07)', s06: 'rgba(255,255,255,0.06)',
  s05: 'rgba(255,255,255,0.05)', s03: 'rgba(255,255,255,0.03)',
  s025: 'rgba(255,255,255,0.025)', divider: 'rgba(255,255,255,0.06)',
};

const LIGHT_T: SlideTokens = {
  isDark: false,
  bg: '#f8fafc',
  t1: '#0f172a', t2: '#1e293b', t3: '#334155',
  a65: 'rgba(15,23,42,0.70)', a60: 'rgba(15,23,42,0.65)',
  a55: 'rgba(15,23,42,0.62)', a50: 'rgba(15,23,42,0.55)',
  a45: 'rgba(15,23,42,0.50)', a40: 'rgba(15,23,42,0.45)',
  a38: 'rgba(15,23,42,0.42)', a22: 'rgba(15,23,42,0.28)',
  s1:  'rgba(0,0,0,0.08)',   s08: 'rgba(0,0,0,0.07)',
  s07: 'rgba(0,0,0,0.08)',   s06: 'rgba(0,0,0,0.05)',
  s05: 'rgba(0,0,0,0.04)',   s03: 'rgba(0,0,0,0.03)',
  s025: 'rgba(0,0,0,0.025)', divider: 'rgba(0,0,0,0.07)',
};

function useT(): SlideTokens {
  return React.useContext(SlideThemeContext) ? DARK_T : LIGHT_T;
}
...
// Structure only — add background via useT() in each slide
...
// Always-dark gradient — section / cover slides
...
// Only used in always-dark slides
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(10px, 1.8vw, 18px)' }}>
      {items.map((item, i) => (
        <div key={i} className={`ps-up ps-d${Math.min(i + 2, 9)}`}
          ...
          <span style={{ color: t.t3, fontSize: 'clamp(1rem, 2.2vw, 1.3rem)', lineHeight: 1.55 }}>
            {item}
          </span>
        </div>
      ))}
...
function SlideTitle({ children, delay = '' }: { children: React.ReactNode; delay?: string }) {
  ...
  return (
    <div className={`ps-up ${delay}`} style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 'clamp(1rem, 2.5vw, 1.8rem)' }}>
      <div style={accentBar} />
      <h2 style={{ ...SG, fontSize: 'clamp(1.7rem, 3.8vw, 3rem)', fontWeight: 800, lineHeight: 1.2, color: t.t1 }}>
        {children}
      </h2>
    </div>
  );
...
/* ─── Section divider factory (always dark) ─────────────────────────────── */
...
const Slide01Cover: React.FC = () => (
  <div style={GRAD_BG}>
    ...
    <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: 820 }}>
      ...
      <div className="ps-up ps-d1" style={badge('#059669', 'rgba(16,185,129,0.12)', 'rgba(16,185,129,0.3)')}>
...
const Slide02Agenda: React.FC = () => {
  ...
  return (
    <div style={{ ...SLIDE_BASE, background: t.bg }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(clamp(260px, 42%, 400px), 1fr))', gap: 10, flex: 1 }}>
        {topics.map(([num, title, desc], i) => (
          <div key={num} className={`ps-up ps-d${Math.min(i + 1, 9)}`}
            style={{ ...panel(t.s07, t.s03), display: 'flex', gap: 14, alignItems: 'center' }}>
            ...
            <div>
              <div style={{ color: t.t1, fontWeight: 700, fontSize: 'clamp(0.9rem, 1.9vw, 1.1rem)' }}>{title}</div>
              <div style={{ color: t.a38, fontSize: 'clamp(0.82rem, 1.6vw, 0.96rem)', marginTop: 3 }}>{desc}</div>
            </div>
          </div>
        ))}
      </div>
      <div className="ps-up ps-d9" style={{ marginTop: 14, ...panel('rgba(16,185,129,0.3)', 'rgba(16,185,129,0.07)'), display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <span style={{ color: '#059669', fontWeight: 700, fontSize: 'clamp(0.9rem, 1.9vw, 1.1rem)' }}>
...
const Slide04Definicion: React.FC = () => {
  ...
  const features = [
    ...
    { color: '#059669', border: 'rgba(110,231,183,0.3)', bg: 'rgba(110,231,183,0.06)', tag: 'Declaración', title: 'Formulario mensual', desc: 'Una vez al mes llenas el formulario SAT-2046 y pagas. Proceso sencillo y digital.' },
    { color: '#2563eb', border: 'rgba(37,99,235,0.35)', bg: 'rgba(37,99,235,0.07)', tag: 'Facturación', title: 'Facturas electrónicas FEL', desc: 'Cada venta debe tener su factura electrónica. Se emite desde el sistema SAT o un certificador.' },
    { color: '#ea580c', border: 'rgba(234,88,12,0.35)', bg: 'rgba(234,88,12,0.07)', tag: 'Límite', title: '~Q 500,000 al año', desc: 'Si ganas más de eso en el año, te debes cambiar a otro régimen tributario.' },
  ];
  return (
    <div style={{ ...SLIDE_BASE, background: t.bg }}>
      ...
      <div className="ps-up ps-d1" style={{ ...panel('rgba(147,51,234,0.35)', 'rgba(147,51,234,0.1)'), marginBottom: 'clamp(0.8rem, 2vw, 1.4rem)', borderLeft: '5px solid #9333ea', borderRadius: '0 12px 12px 0' }}>
        <p style={{ color: t.t2, fontSize: 'clamp(1.05rem, 2.3vw, 1.4rem)', lineHeight: 1.65, margin: 0 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12, flex: 1 }}>
        {features.map((f, i) => (
          <div key={i} className={`ps-up ps-d${Math.min(i + 2, 9)}`}
            ...
            <div style={{ color: t.t1, fontWeight: 700, fontSize: 'clamp(1.05rem, 2.3vw, 1.4rem)', lineHeight: 1.3 }}>{f.title}</div>
            <div style={{ color: t.a50, fontSize: 'clamp(0.9rem, 2vw, 1.15rem)', lineHeight: 1.55 }}>{f.desc}</div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{ ...SLIDE_BASE, background: t.bg }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'clamp(0.9rem, 2vw, 1.15rem)' }}>
          <thead>
            <tr style={{ background: 'linear-gradient(90deg, #6d28d9, #5b21b6)' }}>
              ...
              <th style={{ padding: '12px 16px', textAlign: 'left', color: '#2563eb', fontWeight: 700, fontSize: 'clamp(0.82rem, 1.7vw, 1rem)' }}>Régimen General / Opcional</th>
            </tr>
          </thead>
          <tbody>
            {rows.map(([asp, pc, rg], i) => (
              <tr key={i} style={{ background: i % 2 === 0 ? t.s025 : 'transparent', borderBottom: `1px solid ${t.divider}` }}>
                <td style={{ padding: '12px 16px', color: t.a60, fontWeight: 600 }}>{asp}</td>
                <td style={{ padding: '12px 16px', color: '#c084fc', fontWeight: 600 }}>{pc}</td>
                <td style={{ padding: '12px 16px', color: '#2563eb' }}>{rg}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="ps-up ps-d8" style={{ marginTop: 12, ...panel('rgba(16,185,129,0.3)', 'rgba(16,185,129,0.07)') }}>
        <p style={{ color: '#059669', fontSize: 'clamp(0.95rem, 2vw, 1.2rem)', fontWeight: 600, margin: 0 }}>
    ...
    <div style={{ ...SLIDE_BASE, background: t.bg }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, flex: 1 }}>
        <div className="ps-up ps-d1" style={{ textAlign: 'center', padding: 'clamp(1rem, 3vw, 2rem)', borderRadius: 16, background: 'linear-gradient(135deg, rgba(147,51,234,0.18), rgba(109,40,217,0.1))', border: '1px solid rgba(147,51,234,0.35)' }}>
          <div style={{ color: t.a50, fontSize: 'clamp(0.85rem, 1.8vw, 1rem)', fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 8 }}>
            Límite máximo de ingresos anuales
          </div>
          <div style={{ ...SG, fontSize: 'clamp(3.5rem, 11vw, 7.5rem)', fontWeight: 900, color: '#c084fc', lineHeight: 1 }}>
            ~Q 500,000
          </div>
          <div style={{ color: t.a45, fontSize: 'clamp(0.85rem, 1.8vw, 1rem)', marginTop: 10 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, flex: 1 }}>
          <div className="ps-up ps-d2" style={{ ...panel('rgba(16,185,129,0.3)', 'rgba(16,185,129,0.07)') }}>
            <div style={{ marginBottom: 12 }}>
              <Tag label="Si no superas el límite" color="#059669" />
            </div>
            <BulletList color="#059669" items={[
          ...
          <div className="ps-up ps-d3" style={{ ...panel('rgba(239,68,68,0.3)', 'rgba(239,68,68,0.07)') }}>
            <div style={{ marginBottom: 12 }}>
              <Tag label="Si superas el límite" color="#ef4444" />
            </div>
        ...
        <div className="ps-up ps-d5" style={{ ...panel('rgba(234,88,12,0.35)', 'rgba(234,88,12,0.08)'), display: 'flex', alignItems: 'flex-start', gap: 12 }}>
          <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#ea580c', flexShrink: 0, marginTop: 6 }} />
          <span style={{ color: '#f97316', fontSize: 'clamp(0.95rem, 2vw, 1.2rem)' }}>
...
const Slide09Leyes: React.FC = () => {
  ...
  const leyes = [
    ...
    {
      color: '#059669', border: 'rgba(110,231,183,0.3)', bg: 'rgba(110,231,183,0.07)',
      tag: 'Reforma 2024',
    ...
    {
      color: '#2563eb', border: 'rgba(37,99,235,0.35)', bg: 'rgba(37,99,235,0.08)',
    ...
    {
      color: '#ea580c', border: 'rgba(234,88,12,0.35)', bg: 'rgba(234,88,12,0.08)',
      tag: 'Facturación',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12, flex: 1 }}>
        {leyes.map((l, i) => (
          <div key={i} className={`ps-up ps-d${i + 1}`}
            ...
            <p style={{ color: t.a60, fontSize: 'clamp(0.92rem, 2vw, 1.15rem)', lineHeight: 1.6, margin: 0 }}>
              {l.desc}
            </p>
          </div>
        ))}
    ...
    <div style={{ ...SLIDE_BASE, background: t.bg }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, flex: 1 }}>
        <div className="ps-up ps-d1" style={{ textAlign: 'center', padding: 'clamp(1.4rem, 3.5vw, 2.5rem)', borderRadius: 16, background: 'rgba(147,51,234,0.12)', border: '1px solid rgba(147,51,234,0.35)' }}>
          <div style={{ color: t.a40, fontSize: 'clamp(0.85rem, 1.7vw, 1rem)', fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 'clamp(0.5rem, 2vw, 1.5rem)', flexWrap: 'wrap' }}>
            ...
            <span style={{ ...SG, fontSize: 'clamp(1.8rem, 5vw, 3.8rem)', fontWeight: 300, color: t.a40 }}>=</span>
            <span style={{ ...SG, fontSize: 'clamp(1.8rem, 5vw, 3.8rem)', fontWeight: 700, color: t.t2 }}>Total Ventas del Mes</span>
            <span style={{ ...SG, fontSize: 'clamp(1.8rem, 5vw, 3.8rem)', fontWeight: 300, color: t.a40 }}>×</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, flex: 1 }}>
          <div className="ps-up ps-d2" style={{ ...panel('rgba(239,68,68,0.3)', 'rgba(239,68,68,0.06)') }}>
            <div style={{ marginBottom: 12 }}>
              <Tag label="NO se incluye" color="#ef4444" />
            </div>
            <BulletList color="#ef4444" items={['Tus compras del mes', 'Tus gastos operativos', 'Costo de lo que vendiste']} />
          </div>
          <div className="ps-up ps-d3" style={{ ...panel('rgba(16,185,129,0.3)', 'rgba(16,185,129,0.06)') }}>
            <div style={{ marginBottom: 12 }}>
              <Tag label="SÍ se declara" color="#059669" />
            </div>
            <BulletList color="#059669" items={['Todas las ventas del mes', 'Servicios prestados', 'Total emitido en facturas FEL']} />
          </div>
        </div>
        <div className="ps-up ps-d5" style={{ ...panel('rgba(234,88,12,0.35)', 'rgba(234,88,12,0.08)'), display: 'flex', alignItems: 'flex-start', gap: 12 }}>
          <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#ea580c', flexShrink: 0, marginTop: 6 }} />
  ...
  return (
    <div style={{ ...SLIDE_BASE, background: t.bg }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'clamp(0.9rem, 2vw, 1.15rem)' }}>
          ...
          <tbody>
            {filas.map(([num, fecha, desc, monto], i) => (
              <tr key={i} style={{ borderBottom: `1px solid ${t.divider}`, background: i % 2 === 0 ? t.s025 : 'transparent' }}>
                <td style={{ padding: '10px 14px', color: '#c084fc', fontWeight: 700 }}>{num}</td>
                <td style={{ padding: '10px 14px', color: t.a55 }}>{fecha}</td>
                <td style={{ padding: '10px 14px', color: t.t2 }}>{desc}</td>
                <td style={{ padding: '10px 14px', color: t.t2, textAlign: 'right', fontVariantNumeric: 'tabular-nums' }}>{monto}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
        {[
          ['Total facturado', 'Q 13,150.00', t.t2,    t.s08],
          ['Tasa aplicable',  '× 5%',        '#ea580c','rgba(245,158,11,0.1)'],
          ...
          <div key={i} className={`ps-up ps-d${i + 3}`}
            ...
            <div style={{ color: t.a45, fontSize: 'clamp(0.78rem, 1.6vw, 0.9rem)', fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 10 }}>{label}</div>
  ...
  return (
    <div style={{ ...SLIDE_BASE, background: t.bg }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginTop: 16 }}>
        {[
          ['#ea580c', 'Plazo', 'Hasta el último día del mes siguiente'],
          ['#2563eb', 'Pago', 'BancaSAT · Bancos habilitados · En línea'],
          ['#c084fc', 'Constancia', 'Guárdala siempre — es tu prueba de pago'],
        ].map(([color, label, desc], i) => (
          <div key={i} className={`ps-up ps-d${i + 7}`}
            style={{ ...panel(t.s07, t.s03), textAlign: 'center', borderTop: `3px solid ${color}` }}>
            <div style={{ color, fontWeight: 700, fontSize: 'clamp(0.95rem, 2vw, 1.2rem)', marginTop: 8, marginBottom: 6 }}>{label}</div>
            <div style={{ color: t.a50, fontSize: 'clamp(0.85rem, 1.7vw, 1rem)' }}>{desc}</div>
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{ ...SLIDE_BASE, background: t.bg }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, flex: 1 }}>
        <div className="ps-up ps-d1" style={{ ...panel('rgba(16,185,129,0.35)', 'rgba(16,185,129,0.07)'), display: 'flex', flexDirection: 'column', gap: 14, borderTop: '3px solid #10b981' }}>
          <Tag label="Libro de Ventas — Excel" color="#059669" />
          <p style={{ color: t.a55, fontSize: 'clamp(0.95rem, 2vw, 1.2rem)', margin: 0 }}>
          ...
          <div style={{ borderTop: '1px solid rgba(16,185,129,0.2)', paddingTop: 12 }}>
            <div style={{ color: t.a60, fontWeight: 600, fontSize: 'clamp(0.9rem, 1.8vw, 1.1rem)', marginBottom: 10 }}>Columnas básicas:</div>
            <BulletList color="#059669" items={[
              'Correlativo y fecha',
      ...
      <div className="ps-up ps-d5" style={{ marginTop: 14, ...panel('rgba(234,88,12,0.35)', 'rgba(234,88,12,0.08)'), display: 'flex', gap: 12, alignItems: 'flex-start' }}>
        <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#ea580c', flexShrink: 0, marginTop: 6 }} />
        <p style={{ color: '#f97316', fontSize: 'clamp(0.95rem, 2vw, 1.2rem)', margin: 0 }}>
    ...
    <div style={{ ...SLIDE_BASE, background: t.bg }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, flex: 1 }}>
        <div className="ps-up ps-d1" style={{ ...panel('rgba(239,68,68,0.3)', 'rgba(239,68,68,0.06)'), borderLeft: '5px solid #ef4444', borderRadius: '0 12px 12px 0' }}>
          <p style={{ color: '#ef4444', fontWeight: 700, fontSize: 'clamp(1.05rem, 2.3vw, 1.4rem)', margin: '0 0 6px' }}>
            OMISO = no presentaste tu declaración en el plazo legal
          </p>
          <p style={{ color: t.a50, fontSize: 'clamp(0.95rem, 2vw, 1.2rem)', margin: 0 }}>
            ...
            <thead>
              <tr style={{ background: 'linear-gradient(90deg, #991b1b, #7f1d1d)' }}>
                {['Cargo', 'Cuánto pagas', 'Base legal'].map(h => (
                  <th key={h} style={{ padding: '11px 14px', textAlign: 'left', color: '#ef4444', fontWeight: 700, fontSize: 'clamp(0.85rem, 1.8vw, 1rem)' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {[
                ...
                <tr key={i} style={{ borderBottom: `1px solid ${t.divider}`, background: i % 2 === 0 ? t.s025 : 'transparent' }}>
                  <td style={{ padding: '11px 14px', color: '#ef4444', fontWeight: 700 }}>{cargo}</td>
                  <td style={{ padding: '11px 14px', color: t.t2 }}>{calc}</td>
                  <td style={{ padding: '11px 14px', color: t.a40, fontSize: 'clamp(0.82rem, 1.6vw, 0.95rem)' }}>{base}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <div className="ps-up ps-d5" style={{ ...panel('rgba(234,88,12,0.35)', 'rgba(234,88,12,0.08)'), display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#ea580c', flexShrink: 0, marginTop: 6 }} />
          <p style={{ color: '#f97316', fontSize: 'clamp(0.95rem, 2vw, 1.2rem)', margin: 0 }}>
...
const Slide19EjemploMulta: React.FC = () => {
  ...
  return (
    <div style={{ ...SLIDE_BASE, background: t.bg }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, flex: 1 }}>
        <div className="ps-up ps-d1" style={{ ...panel(t.s07, t.s03) }}>
          <p style={{ color: t.a65, fontSize: 'clamp(1rem, 2.2vw, 1.3rem)', margin: 0 }}>
            <strong style={{ color: t.t2 }}>Situación:</strong> No declaraste el IVA de agosto (ventas: Q 8,000) y lo pagas 20 días tarde.
          </p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, flex: 1 }}>
          <div className="ps-up ps-d2" style={{ ...panel('rgba(147,51,234,0.3)', 'rgba(147,51,234,0.07)'), display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 'clamp(1rem, 2.2vw, 1.3rem)', fontVariantNumeric: 'tabular-nums' }}>
              {[
                ['Ventas del mes', 'Q 8,000.00', t.t2],
                ['IVA correcto (5%)', 'Q 400.00', '#c084fc'],
              ].map(([label, val, color]) => (
                <div key={label} style={{ display: 'flex', justifyContent: 'space-between', borderBottom: `1px solid ${t.s07}`, paddingBottom: 8 }}>
                  <span style={{ color: t.a55 }}>{label}</span>
            ...
            <div style={{ marginTop: 'auto', padding: '10px 14px', borderRadius: 10, background: 'rgba(147,51,234,0.12)', border: '1px solid rgba(147,51,234,0.25)', textAlign: 'center' }}>
              <div style={{ color: t.a45, fontSize: 'clamp(0.85rem, 1.7vw, 1rem)', marginBottom: 4 }}>Si pagas a tiempo</div>
          ...
          <div className="ps-up ps-d3" style={{ ...panel('rgba(239,68,68,0.3)', 'rgba(239,68,68,0.07)'), display: 'flex', flexDirection: 'column', gap: 12 }}>
            <Tag label="Lo que pagas tarde (20 días de atraso)" color="#ef4444" />
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 'clamp(1rem, 2.2vw, 1.3rem)', fontVariantNumeric: 'tabular-nums' }}>
              {[
                ['IVA original', 'Q 400.00', t.t2],
                ['Omiso (20 días × Q 7.50)', 'Q 150.00 (máx)', '#ef4444'],
                ['Intereses (aprox.)', '~Q 8.00', '#dc2626'],
                ['Mora (aprox.)', '~Q 12.00', '#dc2626'],
              ].map(([label, val, color]) => (
                <div key={label} style={{ display: 'flex', justifyContent: 'space-between', borderBottom: `1px solid ${t.s07}`, paddingBottom: 8 }}>
                  <span style={{ color: t.a55 }}>{label}</span>
                  <span style={{ color, fontWeight: 700 }}>{val}</span>
        ...
        <div className="ps-up ps-d5" style={{ ...panel('rgba(239,68,68,0.5)', 'rgba(239,68,68,0.1)'), textAlign: 'center' }}>
          <div style={{ color: t.a50, fontSize: 'clamp(0.9rem, 1.8vw, 1.1rem)', marginBottom: 8 }}>TOTAL A PAGAR (impuesto + cargos)</div>
          <div style={{ ...SG, fontSize: 'clamp(2.5rem, 7vw, 5rem)', fontWeight: 900, color: '#ef4444' }}>Q 570.00</div>
          <div style={{ color: t.a45, fontSize: 'clamp(0.9rem, 1.8vw, 1.1rem)', marginTop: 8 }}>
            vs. Q 400.00 si hubieras pagado a tiempo — <strong style={{ color: '#ef4444' }}>Q 170 extra que podías ahorrarte</strong>
          </div>
        </div>
      </div>
    </div>
...
const Slide21AgenciaVirtual: React.FC = () => {
  ...
  return (
    <div style={{ ...SLIDE_BASE, background: t.bg }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12, flex: 1 }}>
        {[
          { color: '#2563eb', tag: 'Paso 1', title: 'Entrar al portal', desc: 'Ve a portal.sat.gob.gt → Agencia Virtual → ingresa con tu NIT y contraseña.' },
          ...
          { color: '#059669', tag: 'Paso 3', title: 'Filtrar por período', desc: 'Selecciona el tipo de impuesto (IVA) y el mes/año que quieres consultar.' },
          { color: '#ea580c', tag: 'Paso 4', title: 'Revisar el estado', desc: 'GENERADO = llenaste pero no pagaste · PAGADO = todo correcto · OMISO = no presentaste.' },
          ...
          { color: '#dc2626', tag: 'Si hay error', title: 'Llamar a SAT', desc: 'Marca al 1544 o visita el Módulo del Contribuyente. Nunca modifiques los archivos descargados.' },
        ].map((item, i) => (
          <div key={i} className={`ps-up ps-d${Math.min(i + 1, 9)}`}
            style={{ ...panel(t.s07, t.s025), display: 'flex', flexDirection: 'column', gap: 8, borderTop: `3px solid ${item.color}` }}>
            ...
            <p style={{ color: t.a50, fontSize: 'clamp(0.9rem, 1.9vw, 1.1rem)', lineHeight: 1.5, margin: 0 }}>{item.desc}</p>
          </div>
        ))}
      </div>
    </div>
...
const Slide22Resumen: React.FC = () => {
  ...
  const puntos = [
    ...
    ['#2563eb', 'Límite anual', '125 salarios mínimos ≈ Q 500,285/año (Decreto 31-2024). Si lo superas, cambias de régimen.'],
    ['#a78bfa', 'Base legal', 'Decreto 27-92 Arts. 45-50 + Decreto 31-2024 (reforma 2024).'],
    ['#059669', 'Cálculo', 'IVA = Ventas del mes × 5%. Se paga hasta el último día del mes siguiente.'],
    ['#ea580c', 'Formulario SAT-2046', 'Se llena en línea en portal.sat.gob.gt. Guarda siempre la constancia de pago.'],
    ['#db2777', 'LET obligatorio', 'El libro electrónico de ventas se presenta mensualmente en Agencia Virtual.'],
    ['#ef4444', 'Omisos y multas', 'Q 7.50/día · máx Q 150 por declaración · Rectificación Q 15. Declarar a tiempo siempre es más barato.'],
    ['#0284c7', 'Agencia Virtual', 'portal.sat.gob.gt — consulta, descarga y verifica todos tus formularios pagados.'],
  ];
  return (
    <div style={{ ...SLIDE_BASE, background: t.bg }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10, flex: 1 }}>
        {puntos.map(([color, title, desc], i) => (
          <div key={i} className={`ps-up ps-d${Math.min(i + 1, 9)}`}
            style={{ ...panel(t.s07, t.s025), display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            ...
            <div>
              ...
              <div style={{ color: t.a45, fontSize: 'clamp(0.88rem, 1.8vw, 1.05rem)', lineHeight: 1.5 }}>{desc}</div>
            </div>
          </div>
        ))}
      </div>
          ...
          <div className="ps-up ps-d1" style={{ ...panel('rgba(147,51,234,0.3)', 'rgba(147,51,234,0.07)') }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, fontSize: 'clamp(0.95rem, 2vw, 1.15rem)' }}>
              {[
                ...
                <div key={k} style={{ display: 'flex', justifyContent: 'space-between', borderBottom: `1px solid ${t.s05}`, paddingBottom: 6 }}>
                  <span style={{ color: t.a45 }}>{k}</span>
                  <span style={{ color: t.t2, fontWeight: 600 }}>{v}</span>
                </div>
              ))}
            </div>
          </div>
          <div className="ps-up ps-d2" style={{ ...panel(t.s07, t.s025) }}>
            <div style={{ marginBottom: 12 }}>
              <Tag label="Facturas del mes" color={t.t2} />
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 'clamp(0.95rem, 2vw, 1.15rem)', fontVariantNumeric: 'tabular-nums' }}>
              <tbody>
                {[['F-101', 'Q 3,200.00'], ['F-102', 'Q 7,400.00'], ['F-103', 'Q 1,850.00'], ['F-104', 'Q 4,550.00'], ['F-105', 'Q 2,100.00']].map(([f, m]) => (
                  <tr key={f} style={{ borderBottom: `1px solid ${t.s05}` }}>
                    <td style={{ padding: '7px 0', color: '#c084fc', fontWeight: 600 }}>{f}</td>
                    <td style={{ padding: '7px 0', color: t.t2, textAlign: 'right' }}>{m}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div className="ps-up ps-d3" style={{ ...panel('rgba(16,185,129,0.3)', 'rgba(16,185,129,0.07)'), flex: 1 }}>
            <div style={{ marginBottom: 14 }}>
              <Tag label="Lo que debes resolver" color="#059669" />
            </div>
          ...
          <div className="ps-up ps-d8" style={{ ...panel('rgba(234,88,12,0.35)', 'rgba(234,88,12,0.08)'), display: 'flex', gap: 12, alignItems: 'flex-start' }}>
            <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#ea580c', flexShrink: 0, marginTop: 6 }} />
            <p style={{ color: '#f97316', fontSize: 'clamp(0.95rem, 2vw, 1.2rem)', margin: 0, fontWeight: 600 }}>
